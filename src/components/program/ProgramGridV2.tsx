    ...
    <div className="space-y-12 pb-16">
      {timeSlots.map(time => (
        <div key={time} className="relative">
          {/* Временная метка */}
          <div className="sticky top-[120px] z-30 -mx-4 px-4 py-3 mb-6 bg-background/95 backdrop-blur-sm border-b">
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isInPlan ? 'ring-2 ring-primary ring-offset-2 ring-offset-background' : ''}
                  `}
                >
                  {/* Цветовой акцент */}
                    ...
                    {session.tags && session.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {session.tags.slice(0, 3).map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 text-xs font-medium rounded-full bg-background/60"
                          >
                            {tag}
                          </span>
                        ))}
